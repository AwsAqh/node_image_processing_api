...
router.get("/", async (req: Request, res: Response): Promise<void> => {
  Logger.info("Incoming GET request to /thumbs with query:", req.query);
  ...
  try {
    ...
    if (filename) {
      ...
      try {
        ...
        Logger.info(`Thumbnail found: ${filename}`);
        res.json({
          success: true,
    ...
    if (imageFiles.length === 0) {
      Logger.warn("No thumbnails found in folder");
      res.status(404).json({
        success: false,
    ...
    Logger.info(`Returning ${imageFiles.length} thumbnails`);
    res.json({
      success: true,
...
router.get("/:invalidPath", (req: Request, res: Response) => {
  Logger.warn(`Invalid path accessed: ${req.originalUrl}`);
  res.status(400).json({
    success: false,
